...
| `.aw-*` | **Analyst Workspace — Arbiter v1** (`/workspace`) — `.aw-tabs / .aw-tab` 3-tab shell (Queue · Evidence · Watch Note), `.aw-pane` tab body, `.aw-queue` shared table style for queue / scoring-file history / committee inbox / decisions log (14px floor, hover row, `.no-row-click` modifier), `.aw-sf-st` lifecycle status pill (`.draft amber / .submitted info-blue / .returned purple / .approved deploy / .shipped filled-deploy / .rejected dnd`), `.aw-out` outcome chip (same palette, used in committee log rows), `.aw-back` white-fill back-to-queue button, `.aw-sf-hd` scoring-file header strip (name + SF id + status + actions), `.aw-scores / .aw-score` 5 accent-striped dimension cards (svs blue / pps amber / aps orange / rcs amber-watch / ras purple), `.aw-subs / .aw-sub` per-sub-score grid row (label / 0–10 numeric input / `.ev-btn` evidence chip with count badge), `.aw-prv` 3-cell composite preview (Composite · Hard floor · Verdict preview), `.aw-prv-hf` HF cap banner, `.aw-urg` validity urgency chip (expired/urgent/soon/ok), `.aw-view-btn` white-fill row action button, `.aw-ev-list` per-sub-score evidence list inside attach modal, `.aw-hint` greyed empty-state cell text | Current |
| `.cc-*` | **Scoring Committee** (`/committee`) — markup now uses shared `.aw-queue` tables for both Inbox and Decisions log (cc-grid + viewer panel removed; submission viewer moved into the SF detail drawer). Live state hook: `ccState.activeSfId` + decision modal flow | Current |
